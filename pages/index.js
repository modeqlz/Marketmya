...
export default function ProfilePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // 1) Пытаемся взять из local/session storage
      ...
      // 2) Если в Telegram WebApp — делаем «тихий» ре-логин по initData
      try {
        ...
        // проглатываем — попробуем показать ошибку ниже
      }

      // 3) Ничего не получилось
      setBusy(false);
      setError('Нет данных профиля. Вернитесь на главную и войдите заново.');
    })();
  }, []);
  ...
  // Полноэкранный лоадер, пока восстанавливаем сессию
  if (busy) {
    return (
      <>
    ...
    <>
      ...
      <div className="container">
        <div className="hero" style={{maxWidth:560}}>
          ...
          {/* тут дальше добавишь карточки подарков, баланс, аукционы и т.д. */}
        </div>
      </div>
    </>
  );
